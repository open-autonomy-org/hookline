...
1. Copy `container/build-world.json` beside the project and replace the build service's `cwd` with
   ...
   Hermes base and then `hookline-agent:local`. Check `volter-world doctor hookline-image
   --root <world-state-root>`, then `volter-world down hookline-image --root <world-state-root>`
...
3. Run the SDK valve alone on the host to verify connections before activation (the setup guide gives
   ...
   `https://github.com/open-autonomy-org/hookline.git` into `/work/project`. Verify that both effective fetch and push
...
4. Install the reviewed kit's `.open-autonomy` directory in host-owned storage and run:
   ...
   ```bash
   bun .open-autonomy/start.ts --container oa-hookline --secrets /absolute/protected/project-credentials
   ```
